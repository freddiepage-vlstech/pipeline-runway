...
| [0006](0006-pipeline-stage-configuration.md) | Pipeline stage configuration | Accepted 2026-09-21 |
| [0007](0007-stale-opportunity-threshold-configuration.md) | Stale-opportunity threshold configuration | Accepted 2026-09-21 |
| [0008](0008-gateway-enforced-authentication.md) | Gateway-enforced authentication | Accepted 2026-09-21 |

ADR-0002, ADR-0003, and ADR-0001's event-only policy are adapted from Durion Positivity's real production ADRs (ADR-0017, ADR-0056, and ADR-0044, respectively) — reused because they're general-purpose engineering decisions, not because this track needs everything the production platform has. ADR-0004 is adapted from Durion's real "API Artifacts Sync" CI pattern. ADR-0005 resolves REQ-1's own deferred "document the choice in an ADR" acceptance criterion, using ADR-0001's event pattern to do real cross-service business logic. ADR-0006 and ADR-0007 resolve REQ-6's and REQ-10's own equivalent "document this in an ADR" acceptance criteria, surfaced while drafting the OpenAPI contracts (`INFRA-7`) — both were needed before the contracts could stop being tentative on those two points. ADR-0008 resolves a gap surfaced during `companies-service`'s REQ-1 implementation: every OpenAPI contract declares `bearerAuth` security, but nothing enforced it anywhere, and the choice of where to enforce it (per-service vs. the gateway) wasn't settled. See each ADR's Context section for what was adapted vs. skipped.
